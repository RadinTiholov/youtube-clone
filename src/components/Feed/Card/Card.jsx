    ...
    return (
        <div className="col">
            {props.id.videoId
                ...
                : <Link to={`/channel/${props?.id?.channelId}`} style={{ textDecoration: 'none' }}>
                    <div className="card border-0" style={{ width: props.width, borderRadius: 0, background: "#202020"}}>
                        <img className="card-img-top rounded-circle" style={{height: props.height, width: props.width}} src={props?.snippet?.thumbnails.high.url} alt="Video img" />
                        <div className="card-body" style={{ backgroundColor: "#202020" }}>
                            <h6 className="card-text text-light display-7">
                                {props?.snippet?.title}
                            </h6>
